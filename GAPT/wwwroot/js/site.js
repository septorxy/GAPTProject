        ...
        if (type === "updatePlayer") {
            ...
            for (j = 0; j < opponent.length; j++) {
                if (opponent[j].name == inMessage.name) {
                    ...
                    opponent[j].boole = inMessage.boole;
...
function newMessage(x, y, name, speed, boole, direction) {
    this.x = x;
    this.y = y;
    this.name = name;
    this.speed = speed;
...
function component(width, height, color, x, y, type, name) {
    ...
    this.newPos = function () {
        ...
        var newPlayerUpdate = new newMessage(this.x, this.y, this.name, this.speed, this.boole, this.direction);
