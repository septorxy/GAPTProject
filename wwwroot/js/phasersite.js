...
function startGame() { game = new Phaser.Game(config); }
...
function create() {
    ...
    this.opponents = opponent;
        ...
        if (outMessage) {
            ...
            connection.send('broadcastMessage', "newPlayer", outMessage);
        }
...
function update() {
    ...
    {
        if ((!opponent[i].anims.isPlaying) && oppAnim[i] != opponent[i].anims.currentAnim.key) {
    ...
    if (curX != this.player.x || curY != this.player.y) {
        connection.send('broadcastMessage', "updatePlayer", sendMessage(this.player.x, this.player.y, this.player.name, this.player.anims.currentAnim.key));
    }
